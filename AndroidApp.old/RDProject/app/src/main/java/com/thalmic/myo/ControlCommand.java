...
abstract class ControlCommand
{
    ...
    static byte[] createForSetMode(final EmgMode streamEmg, final boolean streamImu, final boolean enableClassifier) {
        byte emgMode = 0;
        switch (streamEmg) {
            case FV: {
                emgMode = 1;
        ...
        final byte imuMode = (byte)(streamImu ? 1 : 0);
        final byte classifierMode = (byte)(enableClassifier ? 1 : 0);
        return createForSetMode(emgMode, imuMode, classifierMode);
    }
    
    private static byte[] createForSetMode(final byte emgMode, final byte imuMode, final byte classifierMode) {
        final byte[] controlCommand = new byte[SetMode.values().length];
    ...
    static byte[] createForVibrate(final Myo.VibrationType vibrationType) {
        final byte[] command = new byte[Vibration.values().length];
    ...
    private static byte getVibrationType(final Myo.VibrationType vibrationType) {
    ...
    static byte[] createForUnlock(final Myo.UnlockType unlockType) {
        final byte[] command = new byte[Unlock.values().length];
    ...
    private static byte getUnlockTypeType(final Myo.UnlockType unlockType) {
        if (unlockType == null) {
            return 0;
        }
        switch (unlockType) {
